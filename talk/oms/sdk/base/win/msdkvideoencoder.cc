/*
Copyright (C) 2108 Intel Corporation. All rights reserved.
...
#include "talk/oms/sdk/base/win/msdkvideobase.h"
#include "talk/oms/sdk/base/win/d3d_allocator.h"
#include <vector>
...
mfxStatus MSDKConvertFrameRate(mfxF64 dFrameRate, mfxU32* pnFrameRateExtN, mfxU32* pnFrameRateExtD) {
  mfxU32 fr;
...
int MSDKVideoEncoder::InitEncodeOnEncoderThread(const webrtc::VideoCodec* codec_settings,
...
mfxU16 MSDKVideoEncoder::MSDKGetFreeSurface(mfxFrameSurface1* pSurfacesPool, mfxU16 nPoolSize) {
...
mfxU16 MSDKVideoEncoder::MSDKGetFreeSurfaceIndex(mfxFrameSurface1* pSurfacesPool, mfxU16 nPoolSize) {
  if (pSurfacesPool) {
...
int MSDKVideoEncoder::Encode(
  ...
  nEncSurfIdx = MSDKGetFreeSurface(m_pEncSurfaces, m_EncResponse.NumFrameActual);
  ...
  ctrl.FrameType = is_keyframe_required ? MFX_FRAMETYPE_I | MFX_FRAMETYPE_REF
                  | MFX_FRAMETYPE_IDR : MFX_FRAMETYPE_P | MFX_FRAMETYPE_REF;
retry:
...
// worker thread.
int MSDKVideoEncoder::EncodeOnEncoderThread(const webrtc::VideoFrame& input_image,
  const webrtc::CodecSpecificInfo* codec_specific_info,
  const std::vector<webrtc::FrameType>* frame_types) {
  mfxStatus sts = MFX_ERR_NONE;
  mfxFrameSurface1* pSurf = NULL; // dispatching pointer
  ...
  while (scPositionsLength < MAX_NALUS_PERFRAME) {
    int32_t naluPosition = NextNaluPosition(encoded_data  + scPosition, 
...
int MSDKVideoEncoder::SetChannelParameters(uint32_t packet_loss,
  int64_t rtt) {
  // Encoder doesn't know anything about packet loss or rtt so just return.
  return WEBRTC_VIDEO_CODEC_OK;
}

int MSDKVideoEncoder::SetRates(uint32_t new_bitrate_kbit,
  uint32_t frame_rate) {
  return WEBRTC_VIDEO_CODEC_OK;
}
...
int32_t MSDKVideoEncoder::NextNaluPosition(
