...
-(instancetype)initWithConfiguration:(RTCPeerClientConfiguration*)configuration signalingChannel:(id<RTCP2PSignalingChannelProtocol>) signalingChannel{
  self=[super init];
...
-(void)connect:(NSString *)token onSuccess:(void (^)(NSString *))onSuccess onFailure:(void (^)(NSError*))onFailure{
  ...
  [_signalingChannel connect:token onSuccess:^(NSString *myId){
...
-(void)disconnectWithOnSuccess:(void (^)())onSuccess onFailure:(void (^)(NSError *))onFailure{
  for (id key in _peerConnectionChannels){
    RTCP2PPeerConnectionChannel* channel = [_peerConnectionChannels objectForKey:key];
...
-(BOOL)checkSignalingChannelOnline:(void(^)(NSError *))failure{
  if(_peerClientState!=kConnected){
    if(failure){
      NSError *err=[[NSError alloc]initWithDomain:RTCErrorDomain code:WoogeenP2PErrorClientInvalidState userInfo:[[NSDictionary alloc]initWithObjectsAndKeys:@"PeerClient haven't connect to a signaling server.", NSLocalizedDescriptionKey, nil]];
      failure(err);
    }
    return NO;
  }
...
-(void)invite:(NSString *)targetId onSuccess:(void (^)())onSuccess onFailure:(void (^)(NSError *))onFailure{
  if(![self checkSignalingChannelOnline:onFailure])
    return;
  RTCP2PPeerConnectionChannel* channel=[self getPeerConnectionChannel:targetId];
  [channel inviteWithOnSuccess:onSuccess onFailure:onFailure];
}


-(void)publish:(RTCLocalStream *)stream to:(NSString *)targetId onSuccess:(void (^)())onSuccess onFailure:(void (^)(NSError *))onFailure{
  if(![self checkSignalingChannelOnline:onFailure])
    return;
  RTCP2PPeerConnectionChannel* channel=[self getPeerConnectionChannel:targetId];
...
-(void)unpublish:(RTCLocalStream *)stream to:(NSString *)targetId onSuccess:(void (^)())onSuccess onFailure:(void (^)(NSError *))onFailure{
  if(![self checkSignalingChannelOnline:onFailure])
    return;
  RTCP2PPeerConnectionChannel* channel=[self getPeerConnectionChannel:targetId];
...
-(RTCP2PPeerConnectionChannel*)getPeerConnectionChannel:(NSString*)targetId{
  RTCP2PPeerConnectionChannel* channel = [_peerConnectionChannels objectForKey:targetId];
  if(channel==nil){
    channel=[[RTCP2PPeerConnectionChannel alloc]initWithConfiguration:_configuration localId:_localId remoteId:targetId signalingSender:self];
...
-(void)sendSignalingMessage:(NSString*)data to:(NSString*)targetId onSuccess:(void (^)())onSuccess onFailure:(void (^)(NSError*)) onFailure{
  [_signalingChannel sendMessage:data to:targetId onSuccess:onSuccess onFailure:onFailure];
}

-(void)stop:(NSString *)targetId onSuccess:(void (^)())onSuccess onFailure:(void (^)(NSError *))onFailure{
  if(![self checkSignalingChannelOnline:onFailure])
    return;
  RTCP2PPeerConnectionChannel* channel=[self getPeerConnectionChannel:targetId];
...
-(void)deny:(NSString *)targetId onSuccess:(void (^)())onSuccess onFailure:(void (^)(NSError *))onFailure{
  if(![self checkSignalingChannelOnline:onFailure])
    return;
  RTCP2PPeerConnectionChannel* channel=[self getPeerConnectionChannel:targetId];
...
-(void)accept:(NSString *)targetId onSuccess:(void (^)())onSuccess onFailure:(void (^)(NSError *))onFailure{
  if(![self checkSignalingChannelOnline:onFailure])
    return;
  RTCP2PPeerConnectionChannel* channel=[self getPeerConnectionChannel:targetId];
...
-(void)send:(NSString *)targetId message:(NSString *)message onSuccess:(void (^)())onSuccess onFailure:(void (^)(NSError *))onFailure{
  if(![self checkSignalingChannelOnline:onFailure])
    return;
  RTCP2PPeerConnectionChannel* channel=[self getPeerConnectionChannel:targetId];
...
-(void)onMessage:(NSString *)message from:(NSString *)senderId{
  RTCP2PPeerConnectionChannel* channel=[self getPeerConnectionChannel:senderId];
