...
void SocketSignalingChannel::AddObserver(
    ConferenceSignalingChannelObserver& observer) {
  observers_.push_back(&observer);
}

void SocketSignalingChannel::RemoveObserver(
    ConferenceSignalingChannelObserver& observer) {
  observers_.erase(std::remove(observers_.begin(), observers_.end(), &observer),
                   observers_.end());
}

void SocketSignalingChannel::Connect(
    ...
    socket_client_->socket()->emit(
        "token", token_message, [=](sio::message::list const& msg) {
          ...
          sio::message::ptr message =
              msg.at(1);  // The second element is room info, please refer to
                          // MCU erizoController's implementation for detailed
                          // message format.
          ...
          for (auto it = stream_messages.begin(); it != stream_messages.end();
               ++it) {
          ...
          for (auto it = user_messages.begin(); it != user_messages.end();
               ++it) {
...
void SocketSignalingChannel::SendSdp(
    Json::Value& options,
    std::string& sdp,
    bool is_publish,
  ...
  rtc::GetStringFromJsonObject(
      options, "video", &video);  // TODO(jianjun): include more information.
  ...
  options_message->get_map()["streamId"] =
      ...
      event_name, message_list, [=](sio::message::list const& msg) {
        ...
        if (event_name == "publish") {
          ...
          if (id->get_flag() != sio::message::flag_string) {
            LOG(LS_WARNING) << "The second element of publish ack is not a "
                               "string. (Expected to be stream ID)";
            return;
          }
          sid = id->get_string();
        }
...
void SocketSignalingChannel::SendStreamEvent(
...
void SocketSignalingChannel::SendCustomMessage(
...
void SocketSignalingChannel::SendStreamControlMessage(
    const std::string& stream_id,
...
Json::Value SocketSignalingChannel::ParseStream(
    const sio::message::ptr& stream_message) {
  Json::Value stream;
  stream["id"] = stream_message->get_map()["id"]->get_string();

  // ugly due to the inconsistency
  if (stream_message->get_map().find("from") !=
      stream_message->get_map().end()) {
  ...
  if (stream_message->get_map()["video"] != nullptr) {
    ...
    if (stream_message->get_map()["video"]->get_flag() ==
...
conference::User SocketSignalingChannel::ParseUser(
    const sio::message::ptr& user_message) {
...
void SocketSignalingChannel::OnEmitAck(
    sio::message::list const& msg,
    std::function<void()> on_success,
    std::function<void(std::unique_ptr<ConferenceException>)> on_failure) {
