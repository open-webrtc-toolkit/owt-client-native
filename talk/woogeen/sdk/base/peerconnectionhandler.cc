...
struct CreateSessionDescriptionMessageData : public rtc::MessageData {
  CreateSessionDescriptionMessageData(
      CreateSessionDescriptionRequest* in_request,
      const webrtc::MediaConstraintsInterface* in_constrants)
      : request(in_request), constrants(in_constrants) {}
  CreateSessionDescriptionRequest* request;
  const webrtc::MediaConstraintsInterface* constrants;
};

void CreateSessionDescriptionRequest::OnSuccess(
    SessionDescriptionInterface* desc) {
  RTC_CHECK(callback_thread_);
  rtc::TypedMessageData<SessionDescriptionInterface*>* message =
      new rtc::TypedMessageData<SessionDescriptionInterface*>(desc);
...
void CreateSessionDescriptionRequest::OnFailure(const std::string& error) {
  RTC_CHECK(callback_thread_);
  rtc::TypedMessageData<std::string>* message =
      new rtc::TypedMessageData<std::string>(error);
...
void CreateSessionDescriptionRequest::OnMessage(Message* msg) {
  switch (msg->message_id) {
    case MSG_CREATESESSIONDESCRIPTION_SUCCESS: {
      rtc::TypedMessageData<SessionDescriptionInterface*>* data =
          static_cast<rtc::TypedMessageData<SessionDescriptionInterface*>*>(
              msg->pdata);
...
void PeerConnectionHandler::OnMessage(Message* msg) {
  switch (msg->message_id) {
    case MSG_CREATEOFFER: {
      CreateSessionDescriptionMessageData* data =
          static_cast<CreateSessionDescriptionMessageData*>(msg->pdata);
...
void PeerConnectionHandler::CreateOffer(
  ...
  CreateSessionDescriptionMessageData* data =
      new CreateSessionDescriptionMessageData(request, constrants);
