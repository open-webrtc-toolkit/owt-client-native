...
class CreateSessionDescriptionRequest
  ...
  // Declare PeerConnectionHandler as friend class so it can set callback
  // thread.
...
// PeerConnectionHandler is a delegate for the WebRTC PeerConnection API
// messages going between client SDK and native PeerConnection in libjingle.
// Methods of this class can be called in any thread.
class PeerConnectionHandler : public rtc::MessageHandler {
 public:
  // All operations on PeerConnection will be performed on |pc_thread| and
  // callbacks will be executed on |callback_thread|.
  explicit PeerConnectionHandler(PeerConnectionInterface* peer_connection,
                                 Thread* pc_thread,
                                 Thread* callback_thread);

  void CreateOffer(CreateSessionDescriptionRequest* request,
                   const webrtc::MediaConstraintsInterface* constrants);

 protected:
  PeerConnectionInterface* peer_connection() { return peer_connection_.get(); }
