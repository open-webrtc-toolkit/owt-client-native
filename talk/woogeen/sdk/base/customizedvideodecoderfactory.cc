...
CustomizedVideoDecoderFactory::CustomizedVideoDecoderFactory(std::unique_ptr<woogeen::base::VideoDecoderInterface> external_decoder)
...
webrtc::VideoDecoder* CustomizedVideoDecoderFactory::CreateVideoDecoder(webrtc::VideoCodecType type) {
  ...
  for (std::vector<webrtc::VideoCodecType>::const_iterator it =
    ...
    ++it) {
      if (*it == type) {
        return new woogeen::base::CustomizedVideoDecoderProxy(type, external_decoder_->Copy());
      }
  }
