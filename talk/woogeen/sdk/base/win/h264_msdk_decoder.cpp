...
H264MSDKVideoDecoder::H264MSDKVideoDecoder(webrtc::VideoCodecType type, HWND decoder_window)
    :codecType_(type),
...
H264MSDKVideoDecoder::~H264MSDKVideoDecoder() {
    ntp_time_ms_.clear();
    timestamps_.clear();
...
bool H264MSDKVideoDecoder::CreateD3DDeviceManager() {
...
int32_t H264MSDKVideoDecoder::InitDecode(const webrtc::VideoCodec* codecSettings, int32_t numberOfCores) {
...
int32_t H264MSDKVideoDecoder::InitDecodeOnCodecThread() {
    ...
    //initialize the decoder for H264. For this sake, we will leave remaining task in Decode call.
...
int32_t H264MSDKVideoDecoder::Decode(
    ...
    while (MFX_ERR_NONE <= sts || MFX_ERR_MORE_SURFACE == sts) {
        mfxU16 moreIdx = H264DecGetFreeSurface(m_pInputSurfaces, m_mfxResponse.NumFrameActual);
...
mfxStatus H264MSDKVideoDecoder::ExtendMfxBitstream(mfxBitstream* pBitstream, mfxU32 nSize) {
...
void H264MSDKVideoDecoder::ReadFromInputStream(mfxBitstream* pBitstream, uint8_t *data, size_t len) {
...
void H264MSDKVideoDecoder::WipeMfxBitstream(mfxBitstream* pBitstream) {
...
mfxU16 H264MSDKVideoDecoder::H264DecGetFreeSurface(mfxFrameSurface1* pSurfacesPool, mfxU16 nPoolSize) {
...
mfxU16 H264MSDKVideoDecoder::H264DecGetFreeSurfaceIndex(mfxFrameSurface1* pSurfacesPool, mfxU16 nPoolSize) {
    if (pSurfacesPool) {
...
int32_t H264MSDKVideoDecoder::RegisterDecodeCompleteCallback(webrtc::DecodedImageCallback* callback) {
    callback_ = callback;
    return WEBRTC_VIDEO_CODEC_OK;
}

void H264MSDKVideoDecoder::OnMessage(rtc::Message* msg) {
